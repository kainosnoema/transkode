...
ImageJob.prototype.processOutput = function processOutput(output, i, callback) {
  ...
  var gm = GM(this.inputStore.stream, this.inputStore.fileName);

  var outStore = Store.parse(output)
  if(!outStore) return callback(new Error('Invalid output'));
  outStore.interpolate(this.meta);
  
  var self = this;
  gm.autoOrient();
  this.applyProcessors(gm, output, function() {
    var format = outStore.format || outStore.fileName;
    gm.stream(format, function(err, stdout, stderr, cmd) {
      // utils.log.info('`' + cmd + '`');
      if(err) return callback(err);
      // need to handle stderr issues
      outStore.saveStream(stdout, function(err) {
        utils.log.info('finished job #' + self.job.id + ', output #' + (i + 1));
        callback(err);
      });
    });
  });
