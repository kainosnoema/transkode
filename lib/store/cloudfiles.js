...
CloudFilesStore.prototype.initReadStream = function(callback) {
  ...
  var self = this;
  this.connection.createContainer(this.containerName, function(err) {
    if(err) callback(err);
    
    self.stream = self.connection.createReadStream(self.containerName, self.name);
    self.streamBuffer = buffer(self.stream);
    self.done = false;
...
CloudFilesStore.prototype.captureReadStream = function(stream, callback) {
  if(!this.containerName) return callback(new Error('cloudfiles container not specified'));
  if(!this.name) return callback(new Error('cloudfiles object name not specified'));
...
CloudFilesStore.prototype.saveStream = function(callback) {
  var opts = {
    remote: this.name
  , stream: this.stream
  , headers: { 'content-length': this.streamBuffer.length() }
  };
  this.connection.addFile(this.containerName, opts, callback);
  this.resumeStream();
}
